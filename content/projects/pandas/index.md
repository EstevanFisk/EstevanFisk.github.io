---
title: Bank Customer Churn EDA and Prediction Part 1
date: 2025-11-01
links:
  - type: Kaggle Version
    url: https://www.kaggle.com/code/bgfootball04/bank-customer-churn-eda-and-prediction-part-1
  - type: GitHub Repo
    url: https://github.com/EstevanFisk/Bank-Customer-Churn-EDA-and-Prediction-Part-1
tags:
  - Python
  - Prediction
  - Feature Engineering
  - Hypothesis Testing
---

Project aims to predict bank customer churn using personal and behavioral data to identify key retention factors and develop targeted strategies, leveraging a Kaggle dataset to build the predictive model.

<!--more-->

This project focuses on predicting bank customer churn (whether a customer will leave or stay) using various personal and behavioral attributes. Customer retention is vital for banks, as keeping current customers is significantly more cost-effective than gaining new ones. By identifying the key factors driving churn, banks can develop targeted loyalty and retention strategies to minimize customer turnover. We will build a predictive model using a dataset from Kaggle containing customer information and behavioral data.


{{< iframe src="no_code_bank-customer-churn-eda-and-prediction-part-1" >}}